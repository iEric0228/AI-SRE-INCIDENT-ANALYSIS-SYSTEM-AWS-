...
from models import (
    StructuredContext, ResourceInfo, AlarmInfo, CompletenessInfo,
    MetricsCollectorOutput, LogsCollectorOutput, DeployContextCollectorOutput
)
...
def track_completeness(event: Dict[str, Any]) -> Dict[str, bool]:
    ...
    """
    return {
        "metrics": "metricsError" not in event and event.get('metrics', {}).get('status') == 'success',
        "logs": "logsError" not in event and event.get('logs', {}).get('status') == 'success',
        "changes": "changesError" not in event and event.get('changes', {}).get('status') == 'success'
    }
