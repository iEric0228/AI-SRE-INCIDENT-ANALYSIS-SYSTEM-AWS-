...
def transform_alarm_event(alarm_event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        timestamp_str = alarm_event.get('time', datetime.now(datetime.UTC).isoformat() if hasattr(datetime, 'UTC') else datetime.utcnow().isoformat())
    ...
    try:
        ...
        response = sns_client.publish(
            ...
            MessageAttributes={
                'incidentId': {
                    'DataType': 'String',
                    'StringValue': incident_event['incidentId']
                },
                'alarmState': {
                    'DataType': 'String',
                    'StringValue': incident_event['alarmState']
                }
            }
        )
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if error_code in ['Throttling', 'ServiceUnavailable', 'InternalError']:
            ...
            # Non-retryable error
            logger.error({
                "message": "Non-retryable AWS error",
                "errorCode": error_code,
                "error": str(e)
            })
            return {
