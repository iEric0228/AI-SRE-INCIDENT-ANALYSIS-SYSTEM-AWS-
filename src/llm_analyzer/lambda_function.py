...
def retrieve_prompt_template(ssm_client, parameter_name: str = '/incident-analysis/prompt-template') -> Dict[str, str]:
    """
...
def invoke_bedrock(bedrock_client, prompt: str, model_id: str = 'anthropic.claude-v2', 
    ...
    try:
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            if all(field in analysis for field in required_fields):
                ...
                if not isinstance(analysis['rootCauseHypothesis'], str) or analysis['rootCauseHypothesis'] is None:
                ...
                if not isinstance(analysis['contributingFactors'], list) or analysis['contributingFactors'] is None:
                    raise ValueError("contributingFactors must be a list")
                
                # recommendedActions must be a list
                if not isinstance(analysis['recommendedActions'], list) or analysis['recommendedActions'] is None:
                ...
                analysis['evidence'] = [str(item) for item in analysis['evidence'] if item is not None]
                analysis['contributingFactors'] = [str(item) for item in analysis['contributingFactors'] if item is not None]
                analysis['recommendedActions'] = [str(item) for item in analysis['recommendedActions'] if item is not None]
        ...
        return {
            'rootCauseHypothesis': response_text[:200] if response_text else "Unable to parse analysis",
            'confidence': 'low',
            'evidence': [],
            'contributingFactors': [],
...
def extract_metadata(llm_response: Dict[str, Any], prompt_version: str, 
