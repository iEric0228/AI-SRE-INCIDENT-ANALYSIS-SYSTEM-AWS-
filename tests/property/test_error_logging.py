...
def resource_arn_strategy(draw):
    ...
    if service == 'lambda':
        resource_name = draw(st.text(alphabet='abcdefghijklmnopqrstuvwxyz-', min_size=5, max_size=20))
        ...
        service_name = draw(st.text(alphabet='abcdefghijklmnopqrstuvwxyz-', min_size=5, max_size=20))
...
def correlation_engine_event_strategy(draw):
    ...
    return {
        'incident': {
            ...
            'resourceArn': draw(resource_arn_strategy()),
            'timestamp': draw(timestamp_strategy()),
            'alarmName': draw(st.text(alphabet='abcdefghijklmnopqrstuvwxyz-', min_size=5, max_size=20)),
...
def validate_error_log_structure(error_log, correlation_id=None):
    ...
    has_traceback = (
        'Traceback' in stack_trace or
        'File "' in stack_trace or
        'line ' in stack_trace or
        'in ' in stack_trace
    )
    ...
    if correlation_id:
        assert error_log['correlationId'] == correlation_id, (
    ...
    has_error_details = (
        'error' in error_log or
        'errorType' in error_log or
        'errorCode' in error_log
    )
...
def test_error_logging_metrics_collector_fatal_error(incident_id):
    ...
    **Validates: Requirement 11.6**
    ...
    with patch('metrics_collector.lambda_function.parse_timestamp') as mock_parse, \
        ...
        assert len(error_logs) > 0, f"Handler must emit at least one error log on unexpected failure. All logs: {log_capture.get_json_logs()}"
...
def test_error_logging_logs_collector_unexpected_error(event):
    ...
    **Validates: Requirement 11.6**
    ...
    with patch('logs_collector.lambda_function.logs_client') as mock_logs, \
        ...
        assert len(error_logs) > 0, f"Handler must emit at least one error log on failure. All logs: {log_capture.get_json_logs()}"
...
def test_error_logging_deploy_context_collector(event):
    ...
    **Validates: Requirement 11.6**
    ...
    with patch('deploy_context_collector.lambda_function.cloudtrail') as mock_ct, \
...
def test_error_logging_correlation_engine_fatal_error(incident_id):
    ...
    **Validates: Requirement 11.6**
    ...
    event = {
        ...
        'logs': {'status': 'success', 'logs': [], 'totalMatches': 0, 'returned': 0, 'collectionDuration': 2.0},
        'changes': {'status': 'success', 'changes': [], 'collectionDuration': 1.8}
    }
    
    # Mock a function to raise unexpected exception during processing
    with patch('correlation_engine.lambda_function.normalize_timestamps') as mock_normalize, \
...
def test_error_logging_llm_analyzer(event):
    ...
    **Validates: Requirement 11.6**
    ...
    with patch('llm_analyzer.lambda_function.get_bedrock_client') as mock_get_bedrock, \
        ...
        mock_ssm.get_parameter.return_value = {
            'Parameter': {
                'Value': 'Test prompt: {structured_context}'
            }
        }
...
def test_error_logging_notification_service(event):
    ...
    **Validates: Requirement 11.6**
    ...
    """
    # Patch the module-level clients (created at import time)
    with patch('notification_service.lambda_function.secrets_manager') as mock_secrets, \
...
def test_error_logging_stack_trace_content(error_message):
    ...
    **Validates: Requirement 11.6**
    ...
    with LogCapture() as log_capture:
        ...
        assert error_message in error_log['stackTrace'], (
            "Stack trace must contain the error message"
        )
        
        # Property: Stack trace must contain file and line information
        assert 'test_error_logging.py' in error_log['stackTrace'], (
            "Stack trace must contain file name"
        )
        
        # Property: Error type must be captured
        assert error_log.get('errorType') == 'ValueError', (
            "Error type must be captured correctly"
        )
