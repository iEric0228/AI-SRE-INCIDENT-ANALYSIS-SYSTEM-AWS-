...
def cloudtrail_event_name_strategy(draw):
    ...
    # Resource types
    resource_types = [
        'Function', 'Instance', 'Service', 'DBInstance', 'Stack',
        'Parameter', 'Alarm', 'Rule', 'Policy', 'Bucket', 'Table',
        'LogGroup', 'Role', 'User', 'Group', 'SecurityGroup'
    ]
...
def test_change_event_classification_is_valid(event_name):
    ...
    **Validates: Requirements 5.2**
    ...
    assert change_type in valid_types, (
        f"Change type must be one of {valid_types}. "
        f"Got: {change_type} for event: {event_name}"
    )
    
    # Property 2: Classification must not be None or empty
    assert change_type is not None, (
        f"Change type must not be None for event: {event_name}"
    )
    assert change_type != '', (
        f"Change type must not be empty string for event: {event_name}"
    )
...
def test_change_event_classification_is_deterministic(event_name):
    ...
    **Validates: Requirements 5.2**
    ...
    assert classification_1 == classification_2 == classification_3, (
        f"Classification should be deterministic. "
        f"Got different results for event: {event_name}"
    )
    ...
    **Validates: Requirements 5.2**
    ...
    for event_name in deployment_events:
        ...
        assert change_type == 'deployment', (
            f"Event {event_name} should be classified as 'deployment', "
...
def test_change_event_classification_batch_consistency(event_names):
    ...
    **Validates: Requirements 5.2**
    ...
    assert classifications == reclassifications, (
