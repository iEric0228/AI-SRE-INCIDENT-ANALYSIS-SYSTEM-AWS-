...
def incident_id_strategy(draw):
    ...
    return draw(st.one_of(
        st.uuids().map(str),
        st.text(min_size=10, max_size=50, alphabet='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_')
    ))
...
def confidence_strategy(draw):
    ...
    return draw(st.sampled_from([Confidence.HIGH, Confidence.MEDIUM, Confidence.LOW, Confidence.NONE]))
...
def test_property_21_slack_message_completeness(analysis_report):
    ...
    **Validates: Requirements 8.1, 8.4, 8.5**
    ...
    assert incident_id_escaped in message_json, \
    ...
    assert expected_severity in message_json, \
        f"Slack message should contain severity '{expected_severity}'"
    
    # 3. Verify root cause hypothesis is present (handle JSON escaping)
    hypothesis_escaped = json.dumps(analysis_report.analysis.root_cause_hypothesis)[1:-1]  # Remove quotes
    assert hypothesis_escaped in message_json, \
        "Slack message should contain root cause hypothesis"
    ...
    for action in analysis_report.analysis.recommended_actions:
        ...
        assert action_escaped in message_json, \
    ...
    assert "View Full Incident Details" in message_json or "incident" in message_json.lower(), \
        "Slack message should contain link to full incident details"
    
    # Verify confidence level is present
    assert confidence_value in message_json or confidence_value.capitalize() in message_json, \
        f"Slack message should contain confidence level '{confidence_value}'"
    
    # Verify timestamp is present
    timestamp_str = analysis_report.timestamp.isoformat() if isinstance(analysis_report.timestamp, datetime) else str(analysis_report.timestamp)
    assert timestamp_str in message_json or str(analysis_report.timestamp) in message_json, \
...
def test_property_21_email_subject_completeness(analysis_report):
    ...
    assert analysis_report.incident_id in subject, \
    ...
    assert expected_severity in subject, \
...
def test_property_21_email_plain_text_completeness(analysis_report):
    ...
    assert analysis_report.incident_id in plain_text, \
    ...
    expected_severity = severity_mapping.get(confidence_value, "Low")
    assert expected_severity in plain_text, \
        f"Email should contain severity '{expected_severity}'"
    
    # 3. Verify root cause hypothesis is present
    hypothesis = analysis_report.analysis.root_cause_hypothesis
    assert hypothesis in plain_text, \
        "Email should contain root cause hypothesis"
    
    # 4. Verify recommended actions are present
    for action in analysis_report.analysis.recommended_actions:
        assert action in plain_text, \
            f"Email should contain recommended action '{action}'"
    
    # 5. Verify link to incident details is present
    assert "View Full Incident Details" in plain_text or "incident" in plain_text.lower(), \
        "Email should contain link to full incident details"
    
    # Verify confidence level is present
    assert confidence_value in plain_text or confidence_value.capitalize() in plain_text, \
        f"Email should contain confidence level '{confidence_value}'"
    
    # Verify timestamp is present
    timestamp_str = str(analysis_report.timestamp)
    assert timestamp_str in plain_text or analysis_report.timestamp.isoformat() in plain_text, \
...
def test_property_21_email_html_completeness(analysis_report):
    ...
    # 1. Verify incident ID is present
    assert analysis_report.incident_id in html, \
    ...
    expected_severity = severity_mapping.get(confidence_value, "Low")
    assert expected_severity in html, \
        f"Email HTML should contain severity '{expected_severity}'"
    
    # 3. Verify root cause hypothesis is present
    hypothesis = analysis_report.analysis.root_cause_hypothesis
    assert hypothesis in html, \
        "Email HTML should contain root cause hypothesis"
    ...
    # 5. Verify link to incident details is present
    assert "View Full Incident Details" in html or "incident" in html.lower(), \
        "Email HTML should contain link to full incident details"
    
    # Verify confidence level is present
    assert confidence_value in html or confidence_value.capitalize() in html, \
        f"Email HTML should contain confidence level '{confidence_value}'"
    
    # Verify timestamp is present
    timestamp_str = str(analysis_report.timestamp)
    assert timestamp_str in html or analysis_report.timestamp.isoformat() in html, \
...
def test_recommended_actions_are_numbered_in_notifications(analysis_report):
    ...
    if num_actions > 0:
        ...
        assert "<ol>" in email_html or "1." in email_html, \
