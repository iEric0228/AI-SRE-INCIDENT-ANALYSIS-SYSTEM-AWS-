...
# Clear any cached lambda_function module first - be aggressive about cleanup
for module_name in list(sys.modules.keys()):
    if 'lambda_function' in module_name:
...
from models import AnalysisReport, Analysis, AnalysisMetadata, Confidence, Status, DeliveryStatus
...
def test_property_22_email_attempted_when_slack_fails(analysis_report, slack_error):
    ...
    **Validates: Requirement 8.6**
    ...
    with patch('lambda_function.secrets_manager') as mock_secrets, \
         patch('lambda_function.sns_client.publish') as mock_sns_publish, \
         patch('lambda_function.requests.post') as mock_post, \
         patch('lambda_function.SNS_TOPIC_ARN', 'arn:aws:sns:us-east-1:123456789012:test-topic'):
        ...
        assert mock_sns_publish.called, \
        ...
        assert delivery_status['slack'] == DeliveryStatus.FAILED.value, \
            "Slack delivery should be marked as failed"
        
        # Verify email is marked as delivered (since we mocked it to succeed)
        assert delivery_status['email'] == DeliveryStatus.DELIVERED.value, \
        ...
        assert 'slackError' in delivery_status, \
            "Slack error should be tracked in delivery status"
        assert delivery_status['slackError'] is not None, \
            "Slack error message should be present"
        
        # Verify email error is not present (since it succeeded)
        assert delivery_status.get('emailError') is None, \
...
def test_both_channels_fail_independently(analysis_report):
    ...
    with patch('lambda_function.secrets_manager') as mock_secrets, \
        ...
        assert mock_sns_publish.called, "Email notification (SNS publish) must be attempted even after Slack fails"
...
def test_notification_duration_tracked_regardless_of_failures(analysis_report):
    ...
    with patch('lambda_function.secrets_manager') as mock_secrets, \
        ...
        assert isinstance(result['notificationDuration'], (int, float)), \
            "Notification duration should be a number"
        assert result['notificationDuration'] >= 0, \
            "Notification duration should be non-negative"
...
def test_slack_retry_exhaustion_still_attempts_email():
    ...
    # Mock AWS clients and Slack webhook
    with patch('lambda_function.secrets_manager') as mock_secrets, \
         patch('lambda_function.sns_client.publish', side_effect=mock_sns_publish), \
         patch('lambda_function.requests.post', side_effect=mock_post), \
         patch('time.sleep'), \
         patch('lambda_function.SNS_TOPIC_ARN', 'arn:aws:sns:us-east-1:123456789012:test-topic'):
...
def test_missing_sns_topic_arn_still_attempts_slack():
    ...
    # Set up environment variables (missing SNS_TOPIC_ARN)
    os.environ['SLACK_SECRET_NAME'] = 'test-slack-webhook'
    os.environ['SNS_TOPIC_ARN'] = ''  # Empty/missing
    os.environ['INCIDENT_STORE_BASE_URL'] = 'https://test.example.com/incident'
