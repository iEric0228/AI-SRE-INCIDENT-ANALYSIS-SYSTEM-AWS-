...
def test_llm_failure_notification_slack(report):
    ...
    assert "recommendedActions" in message_text or "Recommended Actions" in message_text, \
        "Notification must include recommended actions section"
    
    # 6. Confidence must be 'none' or indicate low confidence
    assert "none" in message_lower or "unknown" in message_lower or "low" in message_lower, \
...
def test_llm_failure_notification_email(report):
    ...
    assert email_text is not None, \
        "Email message must be generated even for LLM failure"
    
    assert len(email_text) > 0, \
        "Email message must not be empty"
    
    # 2. Email must include incident ID
    assert report['incidentId'] in email_text, \
        "Email must include incident ID"
    ...
    assert has_failure_indicator, \
        "Email must indicate analysis is unavailable"
    
    # 4. Email must include recommended actions
    assert "RECOMMENDED ACTIONS" in email_text or "recommended actions" in email_lower, \
        "Email must include recommended actions section"
    
    # 5. Email must include manual review recommendation
    assert "manual" in email_lower and "review" in email_lower, \
...
def test_notification_includes_error_context(incident_id, error_type):
    ...
    assert slack_has_context or "error" in slack_text, \
        "Slack notification should include error context"
    
    assert email_has_context or "error" in email_lower, \
...
def test_fallback_report_structure_valid(report):
    ...
    assert hasattr(analysis_report, 'incident_id'), \
        "Fallback report must have incident_id"
    
    assert hasattr(analysis_report, 'analysis'), \
        "Fallback report must have analysis"
    
    assert hasattr(analysis_report.analysis, 'root_cause_hypothesis'), \
        "Fallback report must have root_cause_hypothesis"
    
    assert hasattr(analysis_report.analysis, 'confidence'), \
        "Fallback report must have confidence"
    
    assert hasattr(analysis_report.analysis, 'recommended_actions'), \
    ...
    assert str(confidence_value).lower() == 'none', \
        "Fallback report confidence must be 'none'"
    
    # Must have at least one recommended action
    assert len(analysis_report.analysis.recommended_actions) > 0, \
