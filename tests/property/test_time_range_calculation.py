    ...
    **Validates: Requirements 3.1**
    ...
    if incident_timestamp.tzinfo is not None:
        assert start_time.tzinfo is not None, (
            "Start time should preserve timezone information"
        )
        assert end_time.tzinfo is not None, (
            "End time should preserve timezone information"
        )
...
def test_time_range_idempotent(incident_timestamp):
    ...
    **Validates: Requirements 3.1**
    ...
    assert start_time_1 == start_time_2, (
        "Start time should be consistent across multiple calls"
    )
    assert end_time_1 == end_time_2, (
        "End time should be consistent across multiple calls"
    )
...
def test_time_range_monotonic(timestamp1, timestamp2):
    ...
    **Validates: Requirements 3.1**
    ...
    if timestamp1 == timestamp2:
        ...
        assert start_time_1 < start_time_2, (
            "Earlier timestamp should produce earlier start time"
        )
        assert end_time_1 < end_time_2, (
            "Earlier timestamp should produce earlier end time"
        )
