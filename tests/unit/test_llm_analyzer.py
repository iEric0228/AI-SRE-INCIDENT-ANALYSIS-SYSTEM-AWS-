...
class TestLambdaHandler:
    ...
    def test_successful_bedrock_invocation(self, sample_structured_context, sample_llm_response, 
                                          mock_bedrock_client, mock_ssm_client):
        ...
        mock_ssm_client.get_parameter.return_value = {
            "Parameter": {
                "Value": lambda_function.get_default_prompt_template(),
                "Version": 1
            }
        }
        ...
        bedrock_response["body"].read.return_value = json.dumps({
            "completion": json.dumps(sample_llm_response),
        ...
        with patch('lambda_function.get_bedrock_client', return_value=mock_bedrock_client), \
             patch('lambda_function.get_ssm_client', return_value=mock_ssm_client):
            # Act
    ...
    def test_fallback_on_bedrock_failure(self, sample_structured_context, 
                                        mock_bedrock_client, mock_ssm_client):
    ...
    def test_retryable_error_propagation(self, sample_structured_context, 
                                        mock_bedrock_client, mock_ssm_client):
