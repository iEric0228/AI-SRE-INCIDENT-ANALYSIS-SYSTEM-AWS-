...
class TestDynamoDBTableConfiguration:
    ...
    def test_primary_key_schema(self, main_tf_content):
        ...
        assert re.search(r'attribute\s*\{[^}]*name\s*=\s*"incidentId"[^}]*type\s*=\s*"S"', main_tf_content, re.DOTALL)
        
        # Check timestamp attribute
        assert re.search(r'attribute\s*\{[^}]*name\s*=\s*"timestamp"[^}]*type\s*=\s*"S"', main_tf_content, re.DOTALL)
    ...
    def test_resource_index_configuration(self, main_tf_content):
        ...
        resource_index_pattern = r'global_secondary_index\s*\{[^}]*name\s*=\s*"ResourceIndex"[^}]*\}'
    ...
    def test_severity_index_configuration(self, main_tf_content):
        ...
        severity_index_pattern = r'global_secondary_index\s*\{[^}]*name\s*=\s*"SeverityIndex"[^}]*\}'
    ...
    def test_gsi_attributes_defined(self, main_tf_content):
        ...
        assert re.search(r'attribute\s*\{[^}]*name\s*=\s*"resourceArn"[^}]*type\s*=\s*"S"', main_tf_content, re.DOTALL)
        
        # Verify severity attribute (for SeverityIndex)
        assert re.search(r'attribute\s*\{[^}]*name\s*=\s*"severity"[^}]*type\s*=\s*"S"', main_tf_content, re.DOTALL)
